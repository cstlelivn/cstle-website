...
type Goal = "Family living space" | "Legal secondary suite" | "Rental-income suite" | "Entertainment space" | "Help me plan";
...
const goals: Goal[] = ["Family living space", "Legal secondary suite", "Rental-income suite", "Entertainment space", "Help me plan"];
...
function Choice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={`min-h-[52px] rounded-[14px] border px-4 py-3 text-left transition-all ${selected ? "border-[#191919] bg-[#191919] text-white" : "border-black/10 bg-white text-[#191919] hover:border-black/35"}`}>
    ...
    <div className="min-h-screen bg-white text-[#191919]">
      <section className="bg-gradient-to-b from-[#d9d9d9] to-white px-5 pb-10 pt-7 md:px-10 md:pb-14 md:pt-14">
        ...
        <div className="mx-auto mt-12 max-w-[860px] text-center md:mt-14">
          ...
          <h1 className="mx-auto mt-5 max-w-[760px] font-['Anybody',_sans-serif] text-[34px] leading-[0.98] tracking-[-1.5px] md:text-[54px] md:tracking-[-2.5px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700, hyphens: "none", wordBreak: "normal" }}>
            Plan the basement your home needs next.
          </h1>
          <p className="mx-auto mt-5 max-w-[590px] font-['Anybody',_sans-serif] text-[14px] leading-[1.55] md:text-[17px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600, hyphens: "none", wordBreak: "normal" }}>
            Answer a few focused questions. We’ll review the fit, identify the right next step, and contact you—without a high-pressure sales call.
          </p>
      ...
      <main className="mx-auto max-w-[920px] px-5 pb-24 md:px-10">
        {state === "success" ? (
          <section className="mx-auto mt-4 max-w-[700px] rounded-[24px] bg-[#e6efc1] p-7 text-center md:rounded-[36px] md:p-14">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#191919] text-white"><Check size={22} /></div>
            <h2 className="mt-6 font-['Anybody',_sans-serif] text-[30px] leading-none md:text-[44px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700 }}>Your project is now in Cstle.</h2>
          ...
          <section className="relative mt-4 overflow-hidden rounded-[24px] bg-[#f1f1f1] p-5 md:rounded-[36px] md:p-10">
            ...
            <form onSubmit={submit}>
              {step === 1 && <div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Start with the outcome</p><h2 className="mt-3 font-['Anybody',_sans-serif] text-[27px] leading-[1.05] md:text-[40px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700 }}>What should this basement do for you?</h2><div className="mt-7 grid gap-3 md:grid-cols-2">{goals.map((goal) => <Choice key={goal} selected={form.goal === goal} onClick={() => update("goal", goal)}>{goal}</Choice>)}</div></div>}

              {step === 2 && <div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Project fit</p><h2 className="mt-3 font-['Anybody',_sans-serif] text-[27px] leading-[1.05] md:text-[40px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700 }}>Where are you in the planning process?</h2><label className="mt-7 block font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Project city</label><input value={form.city} onChange={(e) => update("city", e.target.value)} className="mt-2 h-[52px] w-full rounded-[12px] border border-black/10 bg-white px-4 font-['Anybody',_sans-serif] text-[14px] outline-none focus:border-black" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }} /><label className="mt-6 block font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Working investment range</label><div className="mt-2 grid gap-3 md:grid-cols-2">{budgets.map((budget) => <Choice key={budget} selected={form.budgetRange === budget} onClick={() => update("budgetRange", budget)}>{budget}</Choice>)}</div><label className="mt-6 block font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Ideal start</label><div className="mt-2 grid gap-3 md:grid-cols-2">{timelines.map((timeline) => <Choice key={timeline} selected={form.timeline === timeline} onClick={() => update("timeline", timeline)}>{timeline}</Choice>)}</div></div>}

              {step === 3 && <div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Readiness</p><h2 className="mt-3 font-['Anybody',_sans-serif] text-[27px] leading-[1.05] md:text-[40px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700 }}>A few details help us recommend the right next step.</h2><div className="mt-7 space-y-3"><Choice selected={form.ownsProperty} onClick={() => update("ownsProperty", !form.ownsProperty)}>I own or control the property</Choice><Choice selected={form.financingReady} onClick={() => update("financingReady", !form.financingReady)}>My project funds or financing are ready</Choice><Choice selected={form.consultationRequested} onClick={() => update("consultationRequested", !form.consultationRequested)}>I’d like to discuss this with Cstle</Choice></div><label className="mt-7 block font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Anything important we should know? <span className="opacity-50">Optional</span></label><textarea value={form.details} onChange={(e) => update("details", e.target.value)} placeholder="Layout, bedrooms, bathroom, rental goals, permits, or questions…" rows={4} className="mt-2 w-full resize-y rounded-[12px] border border-black/10 bg-white p-4 font-['Anybody',_sans-serif] text-[14px] outline-none focus:border-black" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }} /></div>}

              {step === 4 && <div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">Your next step</p><h2 className="mt-3 font-['Anybody',_sans-serif] text-[27px] leading-[1.05] md:text-[40px]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 700 }}>Where should we send your project follow-up?</h2><p className="mt-3 max-w-[600px] font-['Anybody',_sans-serif] text-[14px] leading-[1.5]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }}>Share either an email address or phone number. A street address is optional until a site visit is appropriate.</p><div className="mt-7 grid gap-4 md:grid-cols-2">{([['firstName','First name *'],['lastName','Last name *'],['email','Email'],['phone','Phone'],['address','Project address (optional)']] as const).map(([key,label]) => <label key={key} className={key === 'address' ? 'md:col-span-2' : ''}><span className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">{label}</span><input type={key === 'email' ? 'email' : key === 'phone' ? 'tel' : 'text'} value={form[key]} onChange={(e) => update(key, e.target.value)} className="mt-2 h-[52px] w-full rounded-[12px] border border-black/10 bg-white px-4 font-['Anybody',_sans-serif] text-[14px] outline-none focus:border-black" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }} /></label>)}</div><input aria-hidden="true" tabIndex={-1} autoComplete="off" value={form.company} onChange={(e) => update("company", e.target.value)} className="absolute -left-[9999px] h-0 w-0" /><button disabled={state === "submitting"} className="mt-7 flex min-h-[54px] w-full items-center justify-center rounded-full bg-[#191919] px-6 font-['Roboto_Mono',_sans-serif] text-[11px] font-bold uppercase text-white transition-opacity hover:opacity-90 disabled:opacity-50">{state === "submitting" ? "Sending…" : "Send my project fit request"}</button><p className="mt-4 text-center font-['Roboto_Mono',_sans-serif] text-[9px] uppercase leading-[1.5] opacity-60">By submitting, you’re asking Cstle to contact you about this project. No marketing list required.</p></div>}
              ...
              {step < 4 && <div className="mt-8 flex items-center justify-between gap-3"><button type="button" onClick={() => setStep((current) => Math.max(1, current - 1))} className={`flex min-h-[48px] items-center gap-2 rounded-full px-4 font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase ${step === 1 ? "invisible" : ""}`}><ChevronLeft size={16} /> Back</button><button type="button" onClick={next} className="flex min-h-[50px] items-center gap-2 rounded-full bg-[#191919] px-6 font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase text-white">Continue <ChevronRight size={16} /></button></div>}
            </form>
          </section>
        )}

        <section className="grid gap-4 py-16 md:grid-cols-3"><div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">01 · Fit review</p><p className="mt-2 font-['Anybody',_sans-serif] text-[14px] leading-[1.45]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }}>We review your goals, location, timing and investment range.</p></div><div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">02 · Focused conversation</p><p className="mt-2 font-['Anybody',_sans-serif] text-[14px] leading-[1.45]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }}>If there’s a fit, the first call starts with your actual project—not a generic sales script.</p></div><div><p className="font-['Roboto_Mono',_sans-serif] text-[10px] font-bold uppercase">03 · Clear next step</p><p className="mt-2 font-['Anybody',_sans-serif] text-[14px] leading-[1.45]" style={{ fontVariationSettings: "'wdth' 135", fontWeight: 600 }}>We identify whether planning, a consultation or a site visit makes sense next.</p></div></section>
      </main>
      <Footer />
    </div>
  );
